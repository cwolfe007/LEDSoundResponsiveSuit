...
class MeteorRain(Animation):
    ...
    def animationStep(self):
        ...
        if super(MeteorRain, self).continueAnimations():
            ...
            for (
                pixel
            ) in self.__previousFrame.iteritems():  # Clear out all of the pixels
                if self.__direction:  # Up
                    if (
                        pixel[0] < self.__startpos
                    ):  # Is this pixel greater than the lasted blacked out pixel?
                        if not self.__randomDecay or (np.random.randint(0, 8) > 1):
                            result = self.fadeToBlack(
                                pixel[0], self.__meteortraildecay, pixel[1]
                            )
            ...
            for j in range(0, self.__meteorsize):
                ...
                if drawpixel:
                    self._Animation__currentPixelFrame.append(
                        (pixeltoverify, self.__color)
                    )
...
class MeteorRainRed(MeteorRain):
    def __init__(self, pixelstart, pixelstop, direction, amp=10):
        ...
        MeteorRain.__init__(
            self, pixelstart, pixelstop, direction, amp, self.__name, self.__color
        )
