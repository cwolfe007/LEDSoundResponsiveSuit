...
class Section:
    def __init__(self, name, pixelstart, pixelstop, direction, anime):
        ...
        self.__name = name
        self.__allowedanimationLifeSpan = (
            1.0  # How long something is allowed to live in the queue
        )
        ...
        if anime == "Step":
            ...
            self.__animationStack.append(
                MeteorRain(self.__pixelstart, self.__pixelstop, self.__direction)
            )
            self.__stackSize += 1
        elif anime == "MeteorRainRed":
            self.__animationStack.append(
                MeteorRainRed(self.__pixelstart, self.__pixelstop, self.__direction)
            )
            self.__stackSize += 1
    ...
    def addAnimation(self, anime, amp=0, range=(0, 0)):
        if not self.__stackSize >= self.__stackLimit:
            ...
            if anime == "Step":
                ...
                animation = Sparkle(
                    self.__pixelstart, self.__pixelstop, self.__direction
                )
            elif anime == "SparkleRed":
                animation = SparkleRed(
                    self.__pixelstart, self.__pixelstop, self.__direction
                )
            elif anime == "SparkleGreen":
                animation = SparkleGreen(
                    self.__pixelstart, self.__pixelstop, self.__direction
                )
            elif anime == "SparkleBlue":
                animation = SparkleBlue(
                    self.__pixelstart, self.__pixelstop, self.__direction
                )
            elif anime == "MeteorRain":
                animation = MeteorRain(
                    self.__pixelstart, self.__pixelstop, self.__direction
                )
            elif anime == "MeteorRainRed":
                animation = MeteorRainRed(
                    self.__pixelstart, self.__pixelstop, self.__direction
                )
            elif anime == "MeteorRainGreen":
                animation = MeteorRainGreen(
                    self.__pixelstart, self.__pixelstop, self.__direction
                )
            elif anime == "MeteorRainBlue":
                animation = MeteorRainBlue(
                    self.__pixelstart, self.__pixelstop, self.__direction
                )
            elif anime == "MeteorRainPink":
                animation = MeteorRainPink(
                    self.__pixelstart, self.__pixelstop, self.__direction
                )
            elif anime == "MeteorRainYellow":
                animation = MeteorRainYellow(
                    self.__pixelstart, self.__pixelstop, self.__direction
                )
    ...
    def getNextAnimation(self):
        currentPos = (
            self.__pixelstart
        )  # Set the current postion in the LED strip to the first pixel
        ...
        while self.__animationStack:  # while stack is not empty
            ...
            animation.setPixelStart(
                currentPos
            )  # Set the latest pixel postion in the animations
            animation.animationStep()  # This Calculates the value of the pixel(s) for the current frame
            for (
                p
            ) in (
                animation.currrentPixelFrame()
            ):  # Append the list of pixels to be sent to the queue
                self.__currentFrame.append(p)

            currentPos = (
                animation.getLastPixel()
            )  # set the current position to the latest pixel in the frame, this will tell the next animation where to start

            self.__temp.append(
                animation
            )  # put the animation into a queue for processing
    ...
    def progressAnimations(
        self,
    ):  # iterate through animations in queue and continue the animattions if not finished or remove the finished ones
        curenttime = time.clock()
        while self.__temp:  # While Temp
            animation = self.__temp.pop()

            animationlifespan = (
                curenttime - animation.getAnimationStartTime()
            )  # Check to see how long the animation has been alive, if past the duration of life span, remove it from the queue

            if not animation.animationFinished() and (
                animationlifespan < self.__allowedanimationLifeSpan
            ):
