...
class Step(Animation):
	def __init__(self,pixelstart,pixelstop,direction,sinetable,amp=5.0,name="Step",color=Color(128,128,128)):
		self.__name = name
		self.__color = color
		newval = int(amp)
		self.__traverse=1
	...
	def animationStep(self):
		...
		if  self._Animation__direction:
			start = self._Animation__pixelstart + (self._Animation__currentpos -  self._Animation__pixelstart) #Where to start drawing pixels
			stop = start+(self.__trail*self._Animation__countUp)#where to stop drawing pixels
...
class StepRed(Step):
	def __init__(self,pixelstart,pixelstop,direction,sinetable,amp=5):
		...
		Step.__init__(self,pixelstart,pixelstop,direction,sinetable,amp,self.__name,self.__color)
