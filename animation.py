...
class Animation(object):
	...
	def __init__(self,name,pixelstart, pixelstop,direction,currentpos, color=Color(128,128,128)): #Direction = False:Down Or True:UpS
	...
	def animationStep(self):
		...
		if self.__currentpos == self.__pixelstop: #Ensure that animation only applies the target pixels
			for pixel in range(self.__pixelstart, self.pixelstop,1,self.__countUp):
				if pixel == self.__currentpos: # Set colors in section that are need for animation
					self.__pixel = pixel
	...
	def printCompletionTime(self):
		print "animation completed in " + str(  time.clock() - self.__tic ) +"s to go accross " + str(self.__pixelstop - self.__pixelstart)
